...
async function run (runtimeEnv) {
  ...
  const deploy_response = await contract.deploy(contract_owner.account);
  console.log(deploy_response);

  const contract_info = await contract.instantiate({"count": 102}, "deploy test", contract_owner.account);
  console.log(contract_info);

  const ex_response = await contract.tx.increment(contract_owner.account);
